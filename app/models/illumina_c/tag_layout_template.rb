class IlluminaC::TagLayoutTemplate < Sequencescape::TagLayoutTemplate
  ...
  def coerce
    extend("illumina_c/tag_layout_template/in_#{self.direction.gsub(/\s+/, '_')}s".camelize.constantize)
    extend("illumina_c/tag_layout_template/walk_#{self.walking_by.gsub(/\s+/, '_')}".camelize.constantize)
    self
  end
